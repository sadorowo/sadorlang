...
module.exports = function (code, line) {
    ...
    if (/^if (.*) {$/g.test(line)) {
        ...
        if (result) return run(to_method(code, line)
            .trim()
            .replace(/\r\n/g, '\n\n')
            .split('\n'))
    } else return 0;
...
function to_method(code, line, check_indents = true) {
    ...
    if (!code.some((line) => /^}$/g.test(line)))
        throw new Failure({ name: 'SyntaxFailure', message: 'missing statement end' })
    ...
    while (!/}/g.test(code[actualLineIndex])) {
        ...
        if (/^return (.*)?$/g.test(Helpers.removeIndents(code[actualLineIndex]))) 
        return functionCode;
        else functionCode += Helpers.removeIndents(code[actualLineIndex]) + '\n'
