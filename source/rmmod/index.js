...
async function main() {
    ...
    if (!files.includes(modName))
        throw new Failure({ name: 'SearchFailure', message: 'cannot find module with this name' })

    const moduleFiles = readdirSync(join('.', modName));
    if (['mod.yaml', 'main.cfs'].some((item) => !moduleFiles.includes(item)))
        throw new Failure({ name: 'StructureFailure', message: 'this directory is not a module' })
